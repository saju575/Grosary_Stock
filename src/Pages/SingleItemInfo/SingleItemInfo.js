...
const SingleItemInfo = () => {
  ...
  const handleDeliverQuantity = async (qInput) => {
    ...
    if (parseInt(qInput) <= newQ) {
      ...
      await axios.post("/soldProducts", { ...sold });
    } else {
  ...
  return (
    <div className="container py-20 mx-auto px-2" style={{ minHeight: "80vh" }}>
      ...
      <div className="flex justify-center">
        <div className="flex flex-col md:flex-row md:max-w-xl rounded-lg bg-white shadow-lg">
          ...
          <div className="p-6 flex flex-col justify-start">
            <h5 className="text-gray-900 text-xl  mb-2 font-bold">{name}</h5>
            <p className="text-gray-700 text-base mb-4">{discription}</p>
            <p className="text-gray-700 text-base mb-4">
              Brand : {supplierName}
            </p>
            <p className="text-gray-600 text-sm pt-2">Quantity : {newQ}</p>
            <p className="text-gray-600 text-sm pt-2">Unit Price : {price}</p>
